...
import { Mail, Lock, User, Building2, ShoppingBag, AlertCircle } from 'lucide-react';
...
export default function RegisterPage() {
  ...
  const submit = async (e: React.FormEvent) => {
    ...
    navigate(role === 'company' ? '/company/login' : '/login');
  };

  return (
    <AuthLayout title="Create Account" subtitle="Join FlyNow and start saving today.">
      <div className="grid grid-2 mb-24" style={{ gap: 8 }}>
        {roles.map((r) => (
      ...
      <p className="text-center text-sm text-muted mt-24">Already have an account? <Link to="/login" style={{ color: 'var(--primary-dark)', fontWeight: 600 }}>User sign in</Link> · <Link to="/company/login" style={{ color: '#E4A817', fontWeight: 600 }}>Company</Link></p>
    </AuthLayout>
  );
}
