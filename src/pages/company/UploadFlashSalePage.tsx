...
export default function UploadFlashSalePage() {
  ...
  // Calculate expiry date automatically (48 hours from now)
  ...
  const submit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      await notificationService.create({
        ...
        title: 'New flash sale submission',
        message: `${company.name} submitted flash sale "${form.title}" for review.`,
        target_role: 'admin',
        ref_id: company.id
      });
  ...
  return (
    <DashboardLayout items={companyNav} brand="Company">
      <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: 8 }}>
        <Zap size={28} color="#D89B17" fill="#D89B17" />
        <h1 style={{ fontSize: 26, margin: 0 }}>New Flash Sale</h1>
      </div>
      <p className="text-muted mb-24">Create a limited-time flash deal. Valid for exactly 48 hours upon approval and displays a countdown timer.</p>
      
      {company?.status !== 'approved' && (
        <div className="badge badge-warning mb-24" style={{ padding: 12, width: '100%' }}>
          <AlertCircle size={16} /> Your company is {company?.status}. Only approved companies can upload.
        </div>
      )}
      
      <form onSubmit={submit} className="grid grid-2" style={{ gap: 24, maxWidth: 900 }}>
        <div className="card card-body">
          <label className="label mb-16">Flash Sale Image</label>
          <label className="flex-col items-center justify-center" style={{ border: '2px dashed var(--border)', borderRadius: 14, padding: 32, cursor: 'pointer', minHeight: 240, background: 'var(--bg)' }}>
            {preview ? <img src={preview} style={{ maxHeight: 200, borderRadius: 10 }} /> : (
              <div className="text-center text-muted">
                ...
                <p>Click to upload flash sale image</p>
                <p className="text-xs mt-8">PNG, JPG up to 5MB</p>
              </div>
            )}
        ...
        <div className="card card-body flex-col gap-16">
          <div className="field">
            <label className="label">Flash Deal Title *</label>
            <input className="input" value={form.title} onChange={(e) => setForm({ ...form, title: e.target.value })} placeholder="e.g. iPhone 15 Pro - Midnight Sale" />
          ...
          <div className="grid grid-2" style={{ gap: 12 }}>
            ...
            <div className="field">
              ...
              <input className="input" placeholder="e.g. 45% OFF" value={form.discount} onChange={(e) => handleDiscountChange(e.target.value)} />
          ...
          <div className="grid grid-2" style={{ gap: 12 }}>
            <div className="field">
              ...
              <input type="number" step="0.01" min="0" className="input" placeholder="e.g. 99.99" value={form.retail_price} onChange={(e) => handleRetailPriceChange(e.target.value)} />
            </div>
            <div className="field">
              <label className="label">Discounted Price ($)</label>
              <input type="number" step="0.01" min="0" className="input" placeholder="e.g. 54.99" value={form.discount_price} onChange={(e) => setForm({ ...form, discount_price: e.target.value })} />
            </div>
          </div>
          ...
          <div className="grid grid-2" style={{ gap: 12 }}>
            <div className="field">
              ...
              <input className="input" value={form.coupon_code} onChange={(e) => setForm({ ...form, coupon_code: e.target.value.toUpperCase() })} />
              {errors.coupon_code && <div className="field-error">{errors.coupon_code}</div>}
            </div>
            <div className="field">
              <label className="label" style={{ color: '#D89B17', fontWeight: 700 }}>Active Period</label>
              <input className="input" value="48 Hours (Fixed)" disabled style={{ background: '#FFFDF9', color: '#D89B17', border: '1.5px solid #FFE08A', fontWeight: 600 }} />
            </div>
          </div>

          <div style={{ background: '#FFF9EB', border: '1px solid #FFE4AD', borderRadius: 10, padding: 12, fontSize: 13, color: '#A16207' }}>
            <strong>⏱️ Auto Expiration Details:</strong> Upon approval by admin, this deal will automatically remain active for exactly 48 hours (estimated end: {displayExpiryTime()}).
          </div>
          
          <div className="field">
            <label className="label">Terms & Conditions</label>
            <textarea className="textarea" value={form.terms} onChange={(e) => setForm({ ...form, terms: e.target.value })} placeholder="Limit 1 per user. Cannot be combined with other offers." />
          </div>
          
          <button className="btn btn-primary" disabled={saving} style={{ background: 'linear-gradient(135deg, #D89B17, #F4B000)', border: 'none', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 8 }}>
            <Zap size={16} fill="#fff" /> {saving ? 'Uploading Flash Deal...' : 'Create Flash Sale'}
          </button>
        </div>
      </form>
    </DashboardLayout>
